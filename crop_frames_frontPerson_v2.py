...
def CropFrames(yolo, meta, frames):
    '''
    Step 1. go through all frames in one video and get the biggest and the most in the middle bbox
    '''
    # x_list = []
    # y_list = []
    ...
    for cur_f, image in enumerate(frames):
        ...
        try:
            person_list.append(r[0])
            center_x_list.append(r[0][-1][0])
            bbox_area_list.append(r[0][-1][2] * r[0][-1][3])
        except:
            continue
    ...
    #  -> 1. Choose top 3 biggest boxes
    # try:
    top_3_bbox = np.argsort(bbox_area_list)[-3:][::-1]

    person_front = np.array(person_list)[np.argmin(top_3_bbox)]
    print(person_front)

    # except:
    #     return []

    #      2. choose the closest bbox
    ...
    # crop a frame
    for image in frames:
        new_image = image[x_min:x_max, x_min:x_max, :]
