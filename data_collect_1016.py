...
def main_loop(video_root, file_count):
    while True:
        ...
        # #print("display_frame show  : ", display_frame.shape)                #######  replace IMSHOW to PRINT
        cv2.waitKey(10)
...
if __name__ == '__main__':
    ...
    #    if video.find('video') is not -1:
    #        cam_num = int(re.findall('\d+', video)[0])
    #        cam_list.append(cam_num)
    #        print(video, cam_num)
    #cap = cv2.VideoCapture(cam_list[-1])
    #'''
